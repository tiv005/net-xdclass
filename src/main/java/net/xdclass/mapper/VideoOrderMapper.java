...
public interface VideoOrderMapper {
     ...
     * @return
     */
    @Insert("INSERT INTO `education_website`.`video_order`" +
            "(`openid`, `out_trade_no`, `state`, `create_time`, `notify_time`, `total_fee`, `nickname`, `head_img`, `video_id`, `video_title`, `video_img`, `user_id`, `ip`, `del`) " +
            "VALUES " +
            "(#{openid}, #{outTradeNo},#{state},#{createTime},#{notifyTime},#{totalFee}, nickname=#{nickname}, #{headImg},#{videoId}, #{videoTitle},#{videoImg},#{userId},#{ip},#{del});")
     ...
     * @return
     */
    @Update("update video_order " +
            "set " +
            "state=#{state},notify_time=#{notifyTime},openid=#{openid} " +
            "where " +
